...
def populate_edit_table(database_data):
    ...
    datHeads = ['type', 'transtype', 'symbol', 'tradedate', 'broker', 'shareamount', 'buyprice', 'sell_price', 'commiss']
    ...
    ('sell_price', tables.Column('Sell Price')), ('commiss', tables.Column('Commission'))]
    ...
    for i in database_data.values():
        ...
        for j in datHeads:
            ...
            if j in i:
                if i[j] is not None:
                    if j == 'buyprice' or j =='commiss' or j == 'sell_price':
                        datDict[j] = '${:,.2f}'.format(i[j])
                    elif j == 'tradedate':
                        datDict[j] = i[j].strftime('%m/%d/%Y')
                    else:
